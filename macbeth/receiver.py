...
if device == 'C3':
    ...
    onboard_pixel = neopixel.NeoPixel(board.NEOPIXEL, 1, pixel_order=neopixel.RGB)
else:
    onboard_pixel = None    
...
def update_pixels(dmx_data):
    ...
    if B_non_dim:
        pwmW.duty_cycle = 65535 if dmx_value_B > 32767 else 0
    else:
        pwmW.duty_cycle = dmx_value_B

    dmx_value_C = (dmx_data[dmx_start_addr + 4] << 8) | dmx_data[dmx_start_addr + 5]
    if C_non_dim:
        pwmG.duty_cycle = 65535 if dmx_value_C > 32767 else 0
    else:
        pwmG.duty_cycle = dmx_value_C

    dmx_value_D = (dmx_data[dmx_start_addr + 6] << 8) | dmx_data[dmx_start_addr + 7]
    if D_non_dim:
        pwmB.duty_cycle = 65535 if dmx_value_D > 32767 else 0
    else:
        pwmB.duty_cycle = dmx_value_D
