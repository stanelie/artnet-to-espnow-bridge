...
# CONFIG
dmx_start_addr = 1 # first address of the receiver (receiver consumes 8 addresses)
A_non_dim = False  # slot A = addr+0/+1 -> Red
B_non_dim = False  # slot B = addr+2/+3 -> Green
C_non_dim = False  # slot C = addr+4/+5 -> Blue
D_non_dim = False  # slot D = addr+6/+7 -> White

channel = 6  # radio scan start channel, set to correct channel to connect faster
device = 'S3' # S3 for S3-zero, C3 for C3-mini
...
def update_pixels(dmx_data):
    ...
    dmx_value_A = (dmx_data[dmx_start_addr + 0] << 8) | dmx_data[dmx_start_addr + 1]
    if A_non_dim:
        pwmR.duty_cycle = 65535 if dmx_value_A > 32767 else 0
    else:
        pwmR.duty_cycle = dmx_value_A

    dmx_value_B = (dmx_data[dmx_start_addr + 2] << 8) | dmx_data[dmx_start_addr + 3]
    if B_non_dim:
        pwmG.duty_cycle = 65535 if dmx_value_B > 32767 else 0
    else:
        pwmG.duty_cycle = dmx_value_B

    dmx_value_C = (dmx_data[dmx_start_addr + 4] << 8) | dmx_data[dmx_start_addr + 5]
    if C_non_dim:
        pwmB.duty_cycle = 65535 if dmx_value_C > 32767 else 0
    else:
        pwmB.duty_cycle = dmx_value_C

    dmx_value_D = (dmx_data[dmx_start_addr + 6] << 8) | dmx_data[dmx_start_addr + 7]
    if D_non_dim:
        pwmW.duty_cycle = 65535 if dmx_value_D > 32767 else 0
    else:
        pwmW.duty_cycle = dmx_value_D
...
if device == 'S3':
    onboard_pixel = neopixel.NeoPixel(board.NEOPIXEL, 1, pixel_order=neopixel.GRB) # onboard neopixel
    # measured hardware output order on this board: D1=Green D2=Blue D3=White D4=Red
    pwmG = pwmio.PWMOut(board.D1, frequency=2500)
    pwmB = pwmio.PWMOut(board.D2, frequency=2500)
    pwmW = pwmio.PWMOut(board.D3, frequency=2500)
    pwmR = pwmio.PWMOut(board.D4, frequency=2500)
else:
